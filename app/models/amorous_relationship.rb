...
class AmorousRelationship < ApplicationRecord
  ...
  validates :start_month, :wedding_month, :end_month, inclusion: { in: 1..12, allow_nil: true }
  validates :start_day, :wedding_day, :end_day, inclusion: { in: 1..31, allow_nil: true }
  validate :years_must_be_in_past
  ...
  def years_must_be_in_past
    if start_year.present? && start_year > Time.zone.now.year
      errors.add(:start_year, "can't be in the future!")
      self.start_year = nil
    end
    if wedding_year.present? && wedding_year > Time.zone.now.year
      errors.add(:wedding_year, "can't be in the future!")
      self.wedding_year = nil
    end
